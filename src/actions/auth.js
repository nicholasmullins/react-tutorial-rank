...
export const loadUser = () => async dispatch => {
  // this calls our setAuthToken() with the current token passed into it
...
export const register = ({ name, email, password }) => async dispatch => {
  ...
  try {
    ...
    dispatch({
      // DISPATCH sends it to the REDUCER! so it goes from actions to reducers to the components/layout
...
export const login = (email, password) => async dispatch => {
  // we are no longer taking an object, we are going to create an object with email and password
  const config = {
    headers: {
      'Content-Type': 'application/json'
    }
